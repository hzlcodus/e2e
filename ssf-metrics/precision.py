...
def find_attr_in_input(attr, attr_line):
    if attr in attr_line:
        return True
    return False
...
def main(gold_or_sample, ace_or_extract):
    ...
    with open('/data/hzlcodus/percentage.txt', 'w') as f:
        ...
        for i in range(len(attr_lines)):
            ...
            # if desc has more than 3 sentences, delete the last sentence
            ...
            for key in syn_dict.keys():
                ...
                if in_desc:
                    total_attr_count += 1
                    in_attr = find_attr_in_input(key, attr_line)
                    if not in_attr:
                        #print(key, 'in desc but not in attr', attr_line, desc)
                        pass
                    else:
                        attr_count += 1
